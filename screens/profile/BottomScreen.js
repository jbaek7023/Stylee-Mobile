...
class BottomScreen extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if(this.props.bottoms !== nextProps.bottoms) {
  ...
  _onEndReachedThreshold = () => {
    ...
    if(nextUri) {
      this.props.fetchBottomNextAll(token, hType, nextUri);
    }
  }

  _onRefresh = () => {
    this.setState({refreshing: true});
    this.props.fetchBottomAll(this.props.token, this.props.hType).then((data)=>{
  ...
  render() {
    ...
    if(this.props.bottoms && this.props.bottoms.length==0) {
      return (
        <View style={{ flex:1, alignItems: 'center', justifyContent: 'center'}}>
          <Image
            fadeDuration={0}
