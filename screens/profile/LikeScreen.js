...
class LikeScreen extends Component {
  ...
  componentDidMount() {
    ...
    let { postType, id } = this.props.navigation.state.params;
    if(postType==1) {
      this.props.fetchLikeListOnOutfit(
        token, hType, id, (users, nextUri) => {
          this.setState({users, nextUri, isLoading: false});
        },
        () => {
      ...
      this.props.fetchLikeListOnCloth(
        token, hType, id, (users, nextUri) => {
          this.setState({users, nextUri, isLoading: false});
        },
        () => {
          ...
          <View rkCardHeader style={styles.left}>
            <View style={styles.content}>
              <View style={styles.contentHeader}>
                <RkText rkType='header3'>Like</RkText>
              </View>
            </View>
          </View>
        </View>
  ...
  _onEndReachedThreshold = () => {
    let { nextUri } = this.state;
    if(nextUri) {
      this.props.fetchLikeNextOnPost(
        token, hType, nextUri, (users, nextUri) => {
          let newUsers = this.state.users.concat(users);
          this.setState({users: newUsers, nextUri, isLoading: false});
        }
      );
    }
  }
  ...
  _renderItem = ({item}) => {
    let { image, name, username, id} = item.user;
    return (
      <TouchableOpacity
...
export default connect(mapStateToProps, actions)(LikeScreen);
