...
class FollowingScreen extends Component {
  ...
  componentDidMount() {
    ...
    this.props.retrieveFollowingsByUser(
      token, hType, userPk, (followings, nextUri) => {
        this.setState({followings, nextUri, isLoading: false});
      }
    );
  }
          ...
          <View rkCardHeader style={styles.left}>
            <View style={styles.content}>
              <View style={styles.contentHeader}>
                <RkText rkType='header3'>Following</RkText>
              </View>
            </View>
          </View>
        </View>
  ...
  _onEndReachedThreshold = () => {
    if(this.state.nextUri) {
      this.props.retrieveFollowingsByUser(
        token, hType, this.state.nextUri, (followings, nextUri) => {
          let newFollowings = this.state.followings.concat(followings);
          this.setState({followings: newFollowings, nextUri, isLoading: false});
        }
      );
    }
  }
...
export default connect(mapStateToProps, actions)(FollowingScreen);
